...
resource "aws_ecs_task_definition" "backend" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "DATABASE_URL",  value = "postgresql://${var.db_username}:${var.db_password}@${aws_db_instance.postgres.address}:5432/urlshortener" },
    ]
    ...
    healthCheck = {
      command     = ["CMD-SHELL", "wget -qO- http://localhost:3333/api/health || exit 1"]
      interval    = 30
      timeout     = 5
      retries     = 3
      startPeriod = 60
