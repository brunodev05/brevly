...
export function CreateLinkForm({ onCreated }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (shortCode && !/^[a-zA-Z0-9_-]{3,20}$/.test(shortCode)) {
      setError('URL encurtada inválida: use 3–20 caracteres (letras, números, - e _)')
      return
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-medium text-zinc-400">
            URL encurtada
            <span className="ml-1.5 text-zinc-600 font-normal">opcional — gerada automaticamente se vazio</span>
          </label>
          <div className="relative">
